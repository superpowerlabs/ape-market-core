...
interface IProfile {
  ...
  function associateAccount(address account, uint256 timestamp, bytes memory signature) external;
  ...
  function encodeForSignature(address addr1, address addr2, uint256 timestamp) external view returns (bytes32);
